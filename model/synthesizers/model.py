...
from model.data_sampler import DataSampler # real_idx를 같이 출력함
from model.CDF_basded_encoding_decoding import DataTransformer
...
class BM(object):
    def __init__(self, output_info, train_data, bins, batch_size, log_frequency=False, cuda=True):
        ...
        self.BM_change_data = torch.empty((self.train_data.shape[0], 0)).to(self._device)
        self.bin_info_list = []
        ...
        for idx, column_info in enumerate(self.output_info):
            if is_continuous_column(column_info):
                ...
                self.BM_change_data = torch.cat((self.BM_change_data, real_bin_onehot), dim=1)
    ...
    def sampling_real_label(self, real_idx):
        return self.BM_change_data[real_idx].float()
    ...
    def sample_continuous_condvec(self, real_idx):
        ...
        cont_mask.scatter_(0, indices, 0)
        ...
        cont_con_origin = self.BM_change_data[real_idx,:]
...
class Generator(Module):
    ...
    def __init__(self, embedding_dim, generator_dim, data_dim, BM):
        ...
        self.cont_cond = Linear(dim, self.BM._n_categories) # 연속형 변수의 연산
        self.cont_cond2 = Linear(self.BM._n_categories, self.BM._n_continuous_columns) # 연속형 변수의 마지막 연산
        # self.conv1d = nn.Conv1d(in_channels = self.BM._n_categories, out_channels = self.BM._n_continuous_columns, kernel_size = 3, stride=1, padding=1)
        self.bn = BatchNorm1d(self.BM._n_categories)
        
        self.cate_final = Linear(dim, data_dim) # 범주형 변수의 연산
        self.cate_final2 = Linear(data_dim, data_dim-self.BM._n_continuous_columns) # 범주형 변수의 원핫인코딩 차원만 나오게
...
class model(BaseSynthesizer):
    ...
    def _cont_cond_loss(self, cont_data, cont_con_origin, cont_mask):
        ...
        for index_ in range(len(self.BM.continuous_num_list)):
    ...
    def fit(self, train_data, discrete_columns=(), epochs=None):
        ...
        """BM 설정"""
        self.BM = BM(self._transformer.output_info_list, train_data, bins=self.bins, batch_size = self._batch_size)
        BM_change_data = self.BM.BM_change_data
        ...
        self._generator = Generator(
            self._embedding_dim + self._data_sampler.dim_cond_vec() + self.BM._n_categories, # 연속형 변수에 대한 condition 추가
            self._generator_dim,
            data_dim,
            self.BM).to(self._device)

        discriminator = Discriminator(
            data_dim + self._data_sampler.dim_cond_vec() + self.BM._n_categories,
            self.info_dim,
            ...
            for id_ in range(steps_per_epoch):
                ...
                for n in range(self._discriminator_steps):
                    ...
                    if condvec is None:
                        ...
                        cont_con, cont_mask, cont_con_origin = self.BM.sample_continuous_condvec(real_idx) # 연속형 변수 조건 뽑기
                        ...
                        real, real_idx = self._data_sampler.sample_data(self._batch_size, col, opt)
                        # real_sem = BM_change_data[real_idx,:]
                        cont_con, cont_mask, cont_con_origin = self.BM.sample_continuous_condvec(real_idx) # 연속형 변수 조건 뽑기
                ...
                fake_continuous_label = self.BM.calculate_fake_label(fakeact)
                if (id_ == 1):
